...
skins.custom_path = minetest.get_modpath("__server_images__")

if skins.custom_path then
	skins.custom_path = skins.custom_path.."/textures/"
	function skins.custom(name)
		if io.open(skins.custom_path.."player_"..name..".png") then
			return "player_"..name
		elseif io.open(skins.custom_path.."character_"..name..".png") then
...
skins.update_player_skin = function(player)
	...
	local skin = skins.custom(name) or skins.skins[name]

	if skins.get_type(skin) == skins.type.SPRITE then
		player:set_properties({
			visual = "upright_sprite",
			textures = {skin..".png",skin.."_back.png"},
			visual_size = {x=1, y=2},
		})
	elseif skins.get_type(skin) == skins.type.MODEL then
		player:set_properties({
			visual = "mesh",
			textures = {skin..".png"},
			visual_size = {x=1, y=1},
